...
def filter_data(data):
    ...
    # ------------------------------------------------------
    print("Filtering NIFTY F&O contracts...")
    filtered = [
        item
        for item in data
        if item.get("segment") == "NSE_FO" and item.get("underlying_symbol") == "NIFTY"
    ]
    print(f"Total NIFTY F&O contracts found: {len(filtered)}")

    if not filtered:
        raise Exception("No NIFTY contracts found.")

    # ------------------------------------------------------
    # Count unique expiries and pick the nearest
    # ------------------------------------------------------
    unique_expiries = set(item["expiry"] for item in filtered)
    print(f"Number of distinct expiries: {len(unique_expiries)}")

    nearest_expiry = min(item["expiry"] for item in filtered)
    expiry_date = datetime.fromtimestamp(nearest_expiry / 1000).strftime("%Y-%m-%d")
    print(f"Selected nearest expiry: {expiry_date} (timestamp: {nearest_expiry})")

    nearest = [item for item in filtered if item["expiry"] == nearest_expiry]
    print(f"Contracts for this expiry: {len(nearest)}")
    ...
    print(f"Total strikes before cleaning: {len(grouped)}")
